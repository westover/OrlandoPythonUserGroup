...
def angled_arrow(origin, angle, length, label, **kwargs):
    """Plot an arrow with a text annotation near the middle.
    Arguments:
    ...
    Returns:
    matplotlib arrow and text objects
    """
	# Arrow
...
arrow1 = angled_arrow(center1, arrow1_angle, radius, '$r_1$', 
    length_includes_head=True, width=0.05, color='blue')
arrow2 = angled_arrow(center2, arrow2_angle, radius, '$r_2$', 
    length_includes_head=True, width=0.05, color='red')

# Text annotation
# NOTE: enclosing text string in $...$ gives us LaTeX formatting!
